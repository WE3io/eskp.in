...
| TSK-127 | Phase transition detection: compare revenue vs costs, alert when Phase 2 eligible | **done** 2026-03-14 | budget-check.js now queries matches.paid_at for real revenue; compares vs INFRA_COST_GBP+API; logs alert when 2 consecutive months covered; INFRA_MONTHLY_GBP added to .env.example |
| TSK-125 | Evaluate Sentry free tier: instrument, test beforeSend PII scrubbing, confirm GDPR suitability | **done** 2026-03-14 | Decision: defer. Pino + uncaughtException handlers (TSK-123/124) cover Phase 1 crash visibility. Sentry adds grouping + alerting but requires US data transfer + DPA entry. Re-evaluate at first 10 external users. |
